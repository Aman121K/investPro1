...
function Header() {
  return (
    <>
      ...
      >
        <Container>
          <Navbar.Brand className={styles.navlinks} href="/"><span className={styles.log1}><b>LOGO</b>IPSUM</span></Navbar.Brand>
          <Navbar.Toggle aria-controls="responsive-navbar-nav" />
          <Navbar.Collapse id="responsive-navbar-nav">
            <Nav className="me-auto">
              {MenuItems.map((items, i) => {
                return (
                  <Nav.Link className={styles.navlinks} key={i} href={items.url}>
                    {items.title}
                  </Nav.Link>
                );
              })}
